...
export class ValidatorService {
 ...
 enteroPositivo(valor: any): boolean {
    // Retorna true si valor es un entero positivo
  ...
  letrasNumerosEspacios(valor: string): boolean {
    const regex = /^[a-zA-Z0-9\s]+$/;
    return regex.test(valor);
  }

  ;
  letrasNumerosEspaciosGuiones(valor: string): boolean {
    const regex = /^[a-zA-Z0-9\s-]+$/;
    return regex.test(valor);
  }
